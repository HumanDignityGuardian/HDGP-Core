...
// applyMetaDefaults fills default scene values when missing (per Integration Spec §2.4).
// For systems without an ethics framework, domain/intent/risk_level are often absent.
func applyMetaDefaults(req *EvaluateRequest) {
	if req.Meta.Scene.Domain == "" {
		req.Meta.Scene.Domain = "general"
	}
	if req.Meta.Scene.Intent == "" {
		req.Meta.Scene.Intent = "chat"
	}
	if req.Meta.Scene.RiskLevel == "" {
		req.Meta.Scene.RiskLevel = "medium"
	}
}
...
type EvaluateResponse struct {
	...
	Report          string            `json:"report,omitempty"` // Human-readable formal report (template-generated, no LLM)
}
