...
**端点**：`POST /hdgp/v1/evaluate`  
**可选查询参数**：
- `include_report=true` 或 `include_report=1`：在 JSON 响应中附加 `report` 字段，包含人可读的正式评估报告（模板生成，不接入 LLM），用于申诉、监管提交与人工复核。详见 `spec/HDGP_INTEGRATION_SPEC.md` 第七、八节。
...
```json
{
  ...
  "report": "string, 可选。仅当 include_report=true 时返回，为人可读的正式评估报告（模板生成）"
}
```
