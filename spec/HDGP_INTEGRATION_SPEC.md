...
本规范**主要面向无伦理框架的普遍项目**：系统中尚未内置结构化伦理层（如宪章、Shield、内容审核管道）的 LLM 集成、推荐、通知、Agent 等系统。对于此类系统，HDGP 作为**首个**伦理检查点接入。对于已有完整伦理框架的项目（如 X-TSOS、Intdone），原则上从整个流程中**节选适用步骤**覆盖，仅对需要与 HDGP 对齐或认证的输出路径应用本流程。
...
| 4. 定义裁决映射 | allow/modify/block/fuse 在本系统的业务含义 | 裁决到动作映射 |
...
当被裁决的系统（或被代理方）对 HDGP 的 verdict 提出申诉，或人类需在“HDGP 的裁决”与“对方的申辩”之间进行审议时，HDGP 的角色是**提供规则与事实依据**，而非参与修辞辩论。HDGP 不接入 LLM，其“声音”是结构化的、可审计的。

### 9.1 HDGP 在申诉/辩论中的定位

- **标准参照**：HDGP 提供裁决所依据的规则 ID、原则 ID、条款引用与审计索引；人类或委员会据此判断“裁决是否合理”，而非比较“谁更能说服人”。
- **事实输出**：触发的规则、对应的宪章条款、裁决理由（来自预设的规则描述模板），以及完整的 request_id / audit 索引，供调阅原始审计记录。
- **不参与修辞**：HDGP 不生成“说服性”文本；其输出为**确定性、模板填充**，与 LLM 无关。

### 9.2 结构化输出（供申诉与人类审议）

Engine 的 `EvaluateResponse` 已包含裁决所需的结构化信息；在申诉场景中，应额外提供便于人类审议的**裁决依据摘要**，格式如下（可与正式报告共用）：
...
为确保技术岗位减少后，非技术人员（包括创始人、监管方、申诉委员会）仍能理解 HDGP 的裁决，自动化流水线的**最后一步**可生成**人可读的正式报告**。报告由**模板填充**生成，**不接入 LLM**，满足确定性与可审计性。
